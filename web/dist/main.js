// Page controller: game data ingestion into OPFS, then engine launch.
...
function failRequests(reason) {
  while (requests.length) requests.shift().reject(new Error(reason));
}
...
$('pick').onclick = async () => {
  try {
    if (!window.showDirectoryPicker)
      throw new Error('this browser has no File System Access API; use Chrome, or ingest from the server');
...
$('pick-http').onclick = async () => {
  try {
    $('pick-http').disabled = true;
    $('ingest-status').textContent = 'scanning game folder...';
    await askWorker({ type: 'ingest-http' });
    $('progress').hidden = true;
    print('ingestion complete');
  } catch (e) {
    print('ingestion failed: ' + e.message);
  } finally {
    $('pick-http').disabled = false;
    await refreshState();
  }
};

fetch('/gamefiles/index.json', { method: 'HEAD' }).then((r) => { $('pick-http').hidden = !r.ok; }).catch(() => {});
...
$('launch').onclick = async () => {
  ...
  const args = ['-nosplash', '-nosound', '-nointro', '-nogameintro', '-fsltx', '/opfs/game/fsgame.ltx'];
  if ($('absolute-cursor').checked) args.push('-i');
  args.push(...$('extra-args').value.split(/\s+/).filter(Boolean));
  print('launching: ' + args.join(' '));

  canvas.width = 1280; // render resolution; the CSS size only scales it for display
  canvas.height = 720;

  const { default: createModule } = await import('./xr_3da.js');
  const module = await createModule({ canvas, arguments: args, print, printErr: print });
  canvas.focus();
...
canvas.addEventListener('click', () => {
  canvas.focus();
  if (!$('absolute-cursor').checked && document.pointerLockElement !== canvas)
...
// Debug helper: copy a file out of OPFS to web/out via the dev server
