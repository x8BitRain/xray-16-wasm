...
async function ingestHttp() {
  const listing = await (await fetch('/gamefiles/index.json')).json();
  const files = listing.filter((f) => {
    const parts = f.path.toLowerCase().split('/');
    return !SKIP_DIRS.has(parts[0]) && !SKIP_EXT.test(parts[parts.length - 1]);
  });
  const game = await gameDir();
  ...
  for (const f of files) {
    ...
    if ((await existingSize(dir, name)) !== f.size)
      await writeBlob(dir, name, await (await fetch('/gamefiles/' + f.path)).blob());
    done += f.size;
    postMessage({ type: 'progress', file: f.path, done, total });
  }
  for (const sub of ['logs', 'savedgames', 'screenshots']) await ensureDir(game, ['_appdata_', sub]);
  await writeBlob(game, '.ingested', new Blob([new Date().toISOString()]));
}
...
onmessage = async (e) => {
  try {
    switch (e.data.type) {
      case 'status': postMessage({ type: 'status', ...(await status()) }); break;
      case 'ingest': await ingest(e.data.dir); postMessage({ type: 'done' }); break;
      case 'ingest-http': await ingestHttp(); postMessage({ type: 'done' }); break;
      case 'sync-engine-data': await syncEngineData(); postMessage({ type: 'done' }); break;
    }
  } catch (err) {
